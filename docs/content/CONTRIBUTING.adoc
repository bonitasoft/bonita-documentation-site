...
* Always add: 
 ** Add the icon with a screen and a monkey wrench. https://github.com/bonitasoft/bonita-doc/blob/2021.2/modules/ROOT/assets/images/images/troubleshooting.png[Here is the link to its location]
 ** Add the title "Troubleshooting" and not "Troubleshoot"
 ** Sub-sections to structure the chapters:
...
* If there are several issues, or if there are several examples of a main issue, each one should be contained in a grey area (as you can see in https://documentation.bonitasoft.com/bonita/2021.2/identity/active-directory-or-ldap-authentication#_troubleshooting[this example]). +
This is how to create such areas:
