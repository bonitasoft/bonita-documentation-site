...
Here is an example of the command to build the partial site for link’s checking:
[source,shell]
----
./build-preview.bash \
...
[TIP]
====
To validate the configuration, it is also possible to check the `test-site/` directory, that includes some URL that have required `htmltest` configuration tuning in the past.
[source,shell]
----
./htmltest -c ./htmltest_bonita-documentation-site.yml ./test-site
----
====
