...
* Antora Atlas site manifest (use to <<validate-xref, validate xref>>)
...
[[validate-xref]]
=== xref validation and partial build

What is a xref? For more details, see the {url-antora-docs}/page/xref/[Antora xref documentation]:
[quote]
...
`xref validation` is done automatically while building an Antora site. The build of the Bonita documentation site is configured to break when a xref validation error occurs.
...
The https://gitlab.com/antora/antora-atlas-extension/[Antora Atlas extension] provides a way to work around this issue by making the references available while validating the xref.

It is done through the use of a site manifest, and it works in two steps
...
Xref validation is done in a specific GitHub workflow when validating PR in documentation content repositories.

To locally use the Atlas extension for xref validation, do the same as in xref:.github/actions/build-pr-site/action.yml[] to install the latest alpha Antora version.
