...
* explain what is xref validation + links to contrib guide
* link to this paragraph in the cache paragraph
...
The https://gitlab.com/antora/antora-atlas-extension/[Antora Altas extension] provides a way to workaround this issue by making the references available while validating the xref. It works in two steps
...
* import the site manifest. It is enabled (TODO link to script) when partially building the site

Special local configuration to use the "import" feature

Atlas in alpha, requires an alpha version of Antora to work. 
